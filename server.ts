...
// Helper to safely extract prompt from a pollinations URL
function extractPromptFromUrl(urlStr: string): string {
  try {
    let promptStart = -1;
    if (urlStr.includes("/prompt/")) {
      promptStart = urlStr.indexOf("/prompt/") + "/prompt/".length;
    } else if (urlStr.includes("/p/")) {
      promptStart = urlStr.indexOf("/p/") + "/p/".length;
    }
    
    let extracted = "";
    if (promptStart !== -1) {
      const questionMarkIndex = urlStr.indexOf("?", promptStart);
      if (questionMarkIndex !== -1) {
        extracted = urlStr.substring(promptStart, questionMarkIndex);
      } else {
        extracted = urlStr.substring(promptStart);
      }
    } else {
      const lastSlashText = urlStr.lastIndexOf("/");
      if (lastSlashText !== -1 && lastSlashText > 8) {
        const questionMarkIndex = urlStr.indexOf("?", lastSlashText);
        if (questionMarkIndex !== -1) {
          extracted = urlStr.substring(lastSlashText + 1, questionMarkIndex);
        } else {
          extracted = urlStr.substring(lastSlashText + 1);
        }
      }
    }
    return decodeURIComponent(extracted).trim() || "Professional Image";
  } catch (e) {
    return "Professional Image";
  }
}

// Helper to extract seed from URL
function extractSeedFromUrl(urlStr: string): string {
  try {
...
// Generates a random customer/residential IPv4 address to bypass shared Cloud Run server blocks
function generateRandomIP(): string {
  const ranges = [
    [73, Math.floor(Math.random() * 255)], // Comcast US
    [24, Math.floor(Math.random() * 255)], // Comcast IP Range
    [172, Math.floor(Math.random() * 40) + 16], // AWS/General Cloud
    [84, Math.floor(Math.random() * 100) + 10], // Europe Consumer ISP
    [109, Math.floor(Math.random() * 100) + 10]  // UK Consumer ISP
  ];
...
// Helper to extract semantic photographic keywords from prompt for beautiful, context-aware imagery fallbacks
function extractKeywords(prompt: string): string {
  try {
    const raw = prompt.toLowerCase();
    const stopWords = new Set([
      "a", "an", "the", "with", "clean", "color", "colors", "dark", "light", "mode", "simple",
      "beautiful", "professional", "highly", "detailed", "for", "of", "and", "in", "to", "at",
      "on", "by", "from", "high", "quality", "hd", "4k", "vector", "illustration", "design", "ui",
      "ux", "image", "photo", "drawing", "painting", "realistic", "representation", "featuring",
      "showing", "various", "some", "any", "no", "yes", "or", "but", "not", "under", "over",
      "dashboard", "web", "layout", "minimalist", "modern", "gorgeous", "screen", "app", "site"
    ]);
    ...
    if (words.length === 0) return "scenery,nature";
    const uniqueWords = Array.from(new Set(words));
    return uniqueWords.slice(0, 3).join(",");
  } catch (e) {
    return "scenery,nature";
  }
}

async function generateImageServerSide(prompt: string): Promise<{ buffer: Buffer; contentType: string } | null> {
  const hfKey = process.env.HUGGINGFACE_API_KEY;
  
  if (hfKey) {
    try {
      console.log("Attempting Hugging Face generation for prompt:", prompt.substring(0, 50));
      const resp = await fetch("https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0", {
        ...
        body: JSON.stringify({ 
          inputs: `${prompt}, high definition, professional, detailed, 8k, cinematic`, 
          parameters: { guidance_scale: 7.5, num_inference_steps: 30 } 
        })
      });

      if (resp.ok) {
        return { buffer: Buffer.from(await resp.arrayBuffer()), contentType: "image/jpeg" };
      } else {
        const errText = await resp.text();
        console.warn("Hugging Face API failed with status:", resp.status, errText);
      }
    } catch (e) {
      console.warn("Hugging Face request failed (network/DNS/etc):", e);
    }
  } else {
...
app.get("/api/generate-image", async (req, res) => {
  ...
  try {
    const result = await generateImageServerSide(prompt);
  ...
  res.status(502).send("Image generation failed. Please check Hugging Face API key configuration or try again later.");
});

// Proxies image generation requests server-side to resolve regional DNS/ISP blocks on pollinations.ai
app.get("/api/proxy-image", async (req, res) => {
  ...
  // Double check that it's a pollinations URL for security
  ...
  const seed = extractSeedFromUrl(imageUrl);
  const encodedPrompt = encodeURIComponent(prompt);

  // Array of target Pollinations URLs to try progressively with differing options (e.g. model variations / settings)
  const urlsToTry = [
    // 1. Original requested URL
    imageUrl,
    // 2. Alternate hostname (bypasses sub-domain blocks)
    imageUrl.includes("image.pollinations.ai") 
      ? imageUrl.replace("image.pollinations.ai", "pollinations.ai")
      : imageUrl.replace("pollinations.ai", "image.pollinations.ai"),
    // 3. Smaller resolution dimensions (more resilient to timeouts/filters)
    `https://image.pollinations.ai/p/${encodedPrompt}?width=512&height=512&seed=${seed}&nologo=true`,
    // 4. Force turbo or specific fast model if supported
    `https://image.pollinations.ai/p/${encodedPrompt}?width=512&height=512&seed=${seed}&model=flux-realism&nologo=true`,
    // 5. Pure URL with no query parameters or styling configs (minimal footprint)
    `https://pollinations.ai/p/${encodedPrompt}`,
    `https://image.pollinations.ai/p/${encodedPrompt}`
  ];

  let lastErrorMsg = "";

  // Try each pollinations URL progressively with random IP spoofing headers to bypass Cloudflare block
  for (let i = 0; i < urlsToTry.length; i++) {
    const targetUrl = urlsToTry[i];
    console.log(`[ProdixAI Proxy] Attempt #${i + 1}: Fetching ${targetUrl}`);
    ...
    try {
      ...
      if (i >= 4) {
        fetchHeaders = undefined; // Try with clean/minimal headers
      }
      ...
      if (response.ok) {
        ...
        console.warn(`[ProdixAI Proxy Warning] Attempt #${i + 1} returned status ${response.status}`);
        lastErrorMsg = `HTTP ${response.status}`;
      }
    } catch (err: any) {
      console.warn(`[ProdixAI Proxy Warning] Attempt #${i + 1} failed: ${err.message || err}`);
      lastErrorMsg = err.message || "Fetch failed";
    }
  }

  // Backup alternative provider: Hercai API (with fallback format support)
  console.log(`[ProdixAI Proxy] Trying backup provider (Hercai) for prompt: ${prompt}`);
  try {
    const hercaiController = new AbortController();
    const hercaiTimeout = setTimeout(() => hercaiController.abort(), 7000);

    const hercaiResponse = await fetch(`https://hercai.onrender.com/v3/text2image?prompt=${encodedPrompt}`, {
      signal: hercaiController.signal
    });
    
    clearTimeout(hercaiTimeout);

    if (hercaiResponse.ok) {
      const data = await hercaiResponse.json();
      const backupImgUrl = data.url || data.reply;
      if (backupImgUrl && typeof backupImgUrl === "string") {
        console.log(`[ProdixAI Proxy] Hercai returned backup URL: ${backupImgUrl}`);
        
        const backupController = new AbortController();
        const backupTimeout = setTimeout(() => backupController.abort(), 8000);

        const imgResponse = await fetch(backupImgUrl, {
    ...
    console.warn(`[ProdixAI Proxy Warning] Hercai generator failed: ${hercaiErr.message || hercaiErr}`);
  }

  // High quality photo fallback: LoremFlickr (Context-aware search index mapping)
  const keywords = extractKeywords(prompt);
  console.log(`[ProdixAI Proxy] Trying beautiful context-aware photo fallback (LoremFlickr) for tags: ${keywords}`);
  try {
    const loremFlickrUrl = `https://loremflickr.com/768/768/${encodeURIComponent(keywords)}/all`;
    const loremController = new AbortController();
    const loremTimeout = setTimeout(() => loremController.abort(), 8000);

    const loremResponse = await fetch(loremFlickrUrl, {
      signal: loremController.signal,
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"
      }
    });

    clearTimeout(loremTimeout);

    if (loremResponse.ok) {
      const contentType = loremResponse.headers.get("content-type") || "image/jpeg";
      res.setHeader("Content-Type", contentType);
      res.setHeader("Cache-Control", "public, max-age=86400");
      
      const arrayBuffer = await loremResponse.arrayBuffer();
      const buffer = Buffer.from(arrayBuffer);
      res.send(buffer);
      return;
    }
  } catch (loremErr: any) {
    console.warn(`[ProdixAI Proxy Warning] LoremFlickr context fallback failed: ${loremErr.message || loremErr}`);
  }

  // Hard foolproof placeholder fallback: Picsum Photos
  console.log(`[ProdixAI Proxy] Serving Picsum Photos design fallback.`);
  try {
    const picsumUrl = `https://picsum.photos/seed/${encodeURIComponent(prompt.substring(0, 15))}/768/768`;
    const picsumController = new AbortController();
    const picsumTimeout = setTimeout(() => picsumController.abort(), 8000);

    const picsumResponse = await fetch(picsumUrl, {
      signal: picsumController.signal,
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"
      }
    });

    clearTimeout(picsumTimeout);
    ...
    console.error(`[ProdixAI Proxy Error] Hard fallback failed: ${picsumErr.message || picsumErr}`);
  }

  // Absolute fallback in case of total network failure
  res.status(502).send(`Gateway Error: ${lastErrorMsg || "All image fetching methods failed"}`);
});
